...
function ConfigForm() {
  ...
  const { register, handleSubmit, watch, setValue, reset } = useForm({
    defaultValues: {
      ...
      min_profit_absolute: null as number | null,
      min_profit_percent: null as number | null,
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="p-8 max-w-4xl mx-auto space-y-8 animate-in fade-in duration-500">
      ...
      <section className="space-y-4">
        <h2 className="section-title"><Activity className="w-3.5 h-3.5" /> Execution Constraints</h2>
        <div className="card border-none shadow-soft grid grid-cols-1 md:grid-cols-3 gap-6">
          <div>
            <label className="stat-label mb-2 block text-amber-600">Margin Multiplier (MIS)</label>
            <input id="margin_multiplier" type="number" step="0.1" min="1" max="10"
              {...register('margin_multiplier', { valueAsNumber: true })} className="form-input font-mono font-bold" />
            <p className="text-[9px] text-slate-400 mt-1.5 uppercase tracking-widest">e.g. 5 = 5x intraday leverage via MIS product</p>
          </div>
          <div>
            <label className="stat-label mb-2 block text-emerald-600">Min Profit (₹ Absolute)</label>
            <input id="min_profit_absolute" type="number" step="10" min="0"
              placeholder="e.g. 200"
              {...register('min_profit_absolute', { valueAsNumber: true })} className="form-input font-mono font-bold" />
            <p className="text-[9px] text-slate-400 mt-1.5 uppercase tracking-widest">Skip trade if target profit below this ₹</p>
          </div>
          <div>
            <label className="stat-label mb-2 block text-emerald-600">Min Profit (% Return)</label>
            <input id="min_profit_percent" type="number" step="0.1" min="0"
              placeholder="e.g. 0.5"
              {...register('min_profit_percent', { valueAsNumber: true })} className="form-input font-mono font-bold" />
            <p className="text-[9px] text-slate-400 mt-1.5 uppercase tracking-widest">Skip trade if target return below this %</p>
          </div>
          <div>
            <label className="stat-label mb-2 block text-rose-600">Default SL (%)</label>
            <input id="default_stop_loss_percent" type="number" step="0.1" min="0" placeholder="e.g. 0.5"
              {...register('default_stop_loss_percent', { valueAsNumber: true })} className="form-input font-mono font-bold" />
            <p className="text-[9px] text-slate-400 mt-1.5 uppercase tracking-widest">Global Fallback Range</p>
          </div>
          <div>
            <label className="stat-label mb-2 block text-emerald-600">Default Target (%)</label>
            <input id="default_target_percent" type="number" step="0.1" min="0" placeholder="e.g. 1.0"
              {...register('default_target_percent', { valueAsNumber: true })} className="form-input font-mono font-bold" />
            <p className="text-[9px] text-slate-400 mt-1.5 uppercase tracking-widest">Global Fallback Range</p>
          </div>
        </div>
      </section>
